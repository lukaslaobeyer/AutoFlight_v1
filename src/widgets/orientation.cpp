...
void Orientation::navdataAvailable(std::shared_ptr<const drone::navdata> nd)
{
	//std::cout << nd->attitude(2) << std::endl;

	yawLabel->setText(QString::number(nd->attitude(2)*(180.0f/M_PI), 'f', 0) + "\u00B0");
	pitchLabel->setText(QString::number(nd->attitude(0)*(180.0f/M_PI), 'f', 0) + "\u00B0");
	rollLabel->setText(QString::number(nd->attitude(1)*(180.0f/M_PI), 'f', 0) + "\u00B0");
	
	yawIndicator->rotate((int) (nd->attitude(2)*(180.0f/M_PI)));
	pitchIndicator->rotate((int) (nd->attitude(0)*(180.0f/M_PI)));
	rollIndicator->rotate((int) (nd->attitude(1)*(180.0f/M_PI)));
}
