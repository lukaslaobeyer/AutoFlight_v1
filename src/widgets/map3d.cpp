...
void Map3D::navdataAvailable(std::shared_ptr<const drone::navdata> nd)
{
	droneYaw = nd->attitude(2);
	droneRoll = nd->attitude(1);
	dronePitch = nd->attitude(0);
	droneAlt = nd->altitude;

	// Calculate position using speed and direction data
	int dT = deltaT();
	float translateX = nd->linearvelocity(0) * 0.001 * dT;
	float translateY = nd->linearvelocity(1) * 0.001 * dT;
